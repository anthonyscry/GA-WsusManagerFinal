...
const MaintenanceView: React.FC<MaintenanceViewProps> = ({ isAirGap }) => {
  ...
  const execute = (op: Operation, params: Record<string, any> = {}) => {
    ...
    if (Object.keys(params).length > 0) {
        loggingService.info(`[PARAMS] ${JSON.stringify(params)}`);
    }
                  ...
                  <div className="p-8 space-y-6">
                      {wizardOp.parameters?.map(p => (
                          <div key={p.id} className="space-y-2">
                              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{p.label}</label>
                              {p.type === 'select' ? (
                                  <select 
                      ...
                      <div className="pt-4 flex gap-3">
                          ...
                          <button 
                            onClick={() => execute(wizardOp, paramValues)}
                            className="flex-[2] py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all"
                          >
                            Confirm Execution
                          </button>
                      </div>
