...
const JobOverlay: React.FC<JobOverlayProps> = ({ jobs }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[200] space-y-3 w-80 animate-slideUp">
      {jobs.map(job => (
        <div key={job.id} className="bg-theme-card border border-blue-500/30 rounded-2xl p-5 shadow-2xl backdrop-blur-xl">
           <div className="flex justify-between items-center mb-3">
              <span className="text-[10px] font-black text-theme-primary uppercase tracking-widest truncate max-w-[180px]">
                 {job.name}
              </span>
           ...
           <div className="h-1.5 w-full bg-theme-secondary rounded-full overflow-hidden">
              <div 
           ...
           <p className="text-[8px] font-bold text-theme-muted uppercase mt-2 tracking-tighter">
